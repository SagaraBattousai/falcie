...
Java_com_calotechnologies_faldroid_blockchain_Blockchain_federate(JNIEnv *env, jobject thiz,
                    jlong blockchain_ptr,
                    jlong block_builder,
                    jobjectArray model_weight_layers,
                    jlongArray model_examples_seen,
                    jobjectArray federated_weights)
{
  ...
  jlong *examples_seen = env->GetLongArrayElements(model_examples_seen, nullptr);

  for (jsize model_index = 0; model_index < participating_models; ++model_index)
  {
    //weight_layers has Kotlin Type Array<FloatBuffer>
    weight_layers = (jobjectArray) //Technically redundant but I like to be clear it's 2D
        env->GetObjectArrayElement(model_weight_layers, model_index);

    num_layers = env->GetArrayLength(weight_layers);

    //TODO: maybe do something to allow constructing with capacity for delta_weights
    ...
    for (jsize layer_index = 0; layer_index < num_layers; ++layer_index)
    {
      ...
      //NOTE: For now we will treat each weight layer as a vector and then optimise from there!
      //update->delta_weights.emplace_back
  ...
  env->ReleaseLongArrayElements(model_examples_seen, examples_seen, JNI_ABORT);

  Blockchain_t *blockchain = reinterpret_cast<Blockchain_t*>(blockchain_ptr);
  blockchain->Add(std::move(block));

  NetworkStructureUpdate_t global_update = blockchain->GetLast().GetGlobalUpdate();


  jsize gu_layer_count = env->GetArrayLength(federated_weights);
  for (jsize gu_layer_index = 0; gu_layer_index < gu_layer_count; ++gu_layer_index)
  {
    ...
    memcpy(gu_weights, global_update.delta_weights[gu_layer_index].Data(), gu_weights_length * sizeof(float));
