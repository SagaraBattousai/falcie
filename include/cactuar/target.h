...
public:
  static constexpr std::uint32_t MinimumDifficulty =
      0x21FFFFFF; // At least I think it's the min

  constexpr Target();

  constexpr Target(std::uint32_t);

  static inline constexpr Target BaseTarget() {
    return Target{Target::BaseDifficulty};
  }

  constexpr inline const std::vector<std::byte> Expand() const;

  constexpr operator std::uint32_t() const { return target; }

  inline std::strong_ordering operator<=>(const std::vector<std::byte> &hash) {
...
private:
  ...
  static constexpr unsigned char TargetExponentShifter =
      4; // because it operates on such

  /// Max target value = 0x21_FF_FF_FF, Min target value (obviously) =
  /// 0x00_00_00_00
  std::uint32_t target;
};
...
constexpr inline const std::vector<std::byte> Target::Expand() const {
  ...
  target_array[static_cast<std::int64_t>(arr_index) - 1] =
      std::byte{(unsigned char)((target >> 8) & 255)};
  target_array[static_cast<std::int64_t>(arr_index) - 2] =
      std::byte{(unsigned char)((target >> 16) & 255)};

  return target_array;
}

inline std::strong_ordering operator<=>(const Target &lhs, const Target &rhs) {
