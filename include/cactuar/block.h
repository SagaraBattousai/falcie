...
// NOTE:- Builder isnt enough to make an abstract base class because they will
// be so varied (AKA concepts were the right idea). However "fluent" builder is
//great here! Althoughv... It could work .. No because concept is better as we
// won't use polymorphism i.e. using an abstract type pointing to a base ....
...
class FALCIE_EXPORT Block {
 public:
  // TODO: Hacky fix as array deletes its default constructor if I dont have a
  // constructor!
  Block() = default;
  ...
  virtual void AddTransaction(
      Transaction*
          transaction);  // are const refs polymorphic? // want a copy right?
 ...
 private:
  Block(std::uint32_t version,
        Target target);  // , cactuar::HashFunction);
  ...
  std::shared_ptr<Blockheader>
      header;  // To allow for passing copy into Blockchain
 ...
 public:
  class Builder {
   public:
    // These probably don't need to ne inline and may be detrimental but like it
    // in the same file
    ...
    Block Genisis() const { return Block(Blockheader::Genisis()); };
...
void Block::AddLocalUpdate(Args&&... args) {
  ...
  // TODO: For now, probably init block with network stucture (and possibly
  // builder) To enfores all updates to be of a compatible network may need to
  // change to make network_structure part of the block constructor if
  // (this->global_update.network_structure.size() == 0)
  //{
  //	this->global_update =
  //GlobalNetworkUpdate(this->local_updates[0].network_structure);
  // }
}
}  // namespace cactuar
